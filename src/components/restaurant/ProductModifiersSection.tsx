...
export function ProductModifiersSection({ 
  ...
  const { data: modifiers = [], isLoading } = useQuery({
    queryKey: ["product-modifiers", restaurantId, productId],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["product-modifiers", restaurantId, productId] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, ...modifier }: any) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-muted-foreground">
            Configure complementos específicos para este produto
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {Object.entries(groupedModifiers).map(([type, items]) => (
            <div key={type}>
              <h4 className="font-medium mb-2 capitalize text-sm">{type}</h4>
